...
class UnitController extends Controller
{
    ...
    public function removePerson($id, Request $request){
        ...
        if ($idItem) {
            ...
            $people = UnitPeople::where('id', $idItem)
            ->where('id_unit', $id)->first();
            if ($people) {
                $people->delete();
            }else{
                $array['error'] = 'Este ID não pertence a esta unidade';
                 return $array;
            }
    ...
    public function removeVehicle($id, Request $request){
        ...
        if ($idItem) {
            ...
            $vehicle = UnitVehicle::where('id', $idItem)
            ->where('id_unit', $id)->first();
            if ($vehicle) {
                $vehicle->delete();
            }else{
